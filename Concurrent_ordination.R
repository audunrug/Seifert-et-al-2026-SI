...
mod_cc_bet <- gllvm(y=Y/20,
                    ...
                    row.eff = "random",
                    lv.formula = ~ Altitude_point + dist_t1 + Name + dist_group*distance_road,
                    num.lv.c = 2,
...
mod_r_bet <- gllvm(y=Y/20,
                    X=data_env,
                    family = "orderedBeta", # ordbet
                    #row.eff = "random",
                    formula = ~ Altitude_point + dist_t1 + Name + dist_group*distance_road,
                    num.lv = 2,
                    disp.formula = rep(1, ncol(Y)),
                    n.init =5,
                    trace=T,
...
## summary
summary(mod_cc_bet, by="terms") # hmm

## standard ordinaition plot
ordiplot(mod_cc_bet, symbols=T, type="marginal", biplot=F, 
