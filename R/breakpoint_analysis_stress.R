...
WFvasc_summary <- WFvasc %>%
  group_by(Name, Plot) %>%
  summarise(weighted_stress = weighted.mean(Str, Abundance), .groups = "drop")
WFvasc_final <- left_join(WFvasc, WFvasc_summary, by = c("Name", "Plot"))
WFvasc_final$weighted_stress100 <- WFvasc_final$weighted_stress / 100
...
# Prepare  plot for CWM stress vs distance to road (dist_R)
plot(1, type = "n",
     xlim = c(0,120),
     ylim = c(0.5,1),
...
for (site in site_names) {
  ...
  points(data_subset$dist_R, data_subset$weighted_stress100,
         pch = shapes[site],
         col = adjustcolor(colors[site], alpha.f = 0.25))
  
  # Fit segmented model
  lm_mod <- lm(weighted_stress100 ~ dist_R, data = data_subset)
  ...
  # Console output
  before_bp <- data_subset$weighted_stress100[data_subset$dist_R < bp]
  after_bp <- data_subset$weighted_stress100[data_subset$dist_R >= bp]
...
# Prepare  plot for CWM stress vs distance to turbine (dist_T1)
plot(1, type = "n",
     xlim = c(0,600),
     ylim = c(0.5,1),
...
for (site in site_names) {
  ...
  # Add points with site-specific shape
  points(data_subset$dist_T1, data_subset$weighted_stress100,
         pch = shapes[site],
         col = adjustcolor(colors[site], alpha.f = 0.25))
  
  # Fit segmented model
  lm_mod <- lm(weighted_stress100 ~ dist_T1, data = data_subset)
  ...
  # Console output
  before_bp <- data_subset$weighted_stress100[data_subset$dist_T1 < bp]
  after_bp <- data_subset$weighted_stress100[data_subset$dist_T1 >= bp]
