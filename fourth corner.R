...
data_env_tr <- model.matrix(data=data_env, ~distance_road*dist_group + dist_t1 + Name) |> 
  as.data.frame()
...
data_tr_f <- data_tr |> select(Rud, Com, L, R, N)
fourth_corner_x <- gllvm(data_sp_4th/20,
                         X=data_env_tr,
                         #row.eff = "random",
                         TR=data_tr_f,
                         family = "orderedBeta",
                         formula = ~ (Name + dist_group + distance_road + distX50 + distX1050 + dist_t1) + (Name + dist_group + distance_road + distX50 + distX1050 + dist_t1):(Rud + Com + L + R + N),
                         num.lv = 0,
...
# make gg levelplot
fc_terms <- coefplot(fourth_corner_x, which.Xcoef = 9:44, get.values = T)
fc_terms_1 <- as.data.frame(fourth_corner_x$fourth.corner)
fc_terms_1[7,] <- fc_terms_1[6,] + fc_terms_1[7,] # add slopes for distance_road
fc_terms_1[8,] <- fc_terms_1[6,] + fc_terms_1[8,]
